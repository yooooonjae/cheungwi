...
| D4 | 인출은 월초, 그 달 이자 = **인출 후** 잔액 × 금리/12 | 인출 전 잔액(=이자 3.3495억 작다) |
...
| D8 | LLCR 은 **두 값을 함께** 낸다 — `llcr`(매각가 포함)과 `llcr_noi_only`(잔여 NOI 만), 둘 다 준공 시점·월이율 할인 | 하나만 내기(의미가 갈린다) |
| D9 | margin = profit ÷ **총사업비** | 매각가 대비(같은 이익이 작아 보인다) |
