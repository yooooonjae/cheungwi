...
## 설계 결정 아홉(브리프가 열어 둔 자리 — 여기서 못박는다)
...
| D8 | LLCR 은 **두 값을 함께** 낸다(아래)          | 하나만 내기(의미가 갈린다)       |
| D9 | margin = profit ÷ **총사업비**                | 매각가 대비(같은 이익이 작아 보임)|
...
D4·D7 은 둘 다 **보수적인 쪽**(이자를 크게, 유입을 늦게)이다.

### D8 — LLCR 을 두 값으로 내는 이유

    llcr           = [Σ NOI_k/(1+r/12)^k + 매각가/(1+r/12)^L] ÷ 대출원금
    llcr_noi_only  = [Σ NOI_k/(1+r/12)^k]                     ÷ 대출원금

둘 다 준공 시점 기준, 할인율은 월이율(금리/12)이다. **어느 하나가 다른 하나의
개선판이 아니다 — 묻는 것이 다르다.**

`llcr` 은 이 대출의 실제 상환 재원을 센다. 준공 후 12개월 만에 파는 브리지형
PF 에서 원금을 갚는 돈은 임대수입이 아니라 매각대금이다. 대신 **분자의 97.3%
(G-PF-001)가 매각가라 커버리지 배수가 아니라 사실상 exit-LTV 의 역수처럼
움직인다** — 매각가가 10% 빠지면 이 값도 정확히 0.903배가 된다. 커버리지라는
이름값을 그대로 믿으면 안 되는 지점이다.

`llcr_noi_only` 는 계획(브리프)이 적어 둔 문면 그대로 **잔여 NOI 만** 센다.
대주 관점의 관행적 LLCR 이 잔존가치를 빼는 쪽이라 이 값이 관행에 가깝지만,
lease-up 12개월이라는 유한 horizon 에서는 G-PF-001 기준 **0.0434** 로 1 을
한참 밑돈다. 낮다고 사업이 부실한 것이 아니라, **운영수입만으로는 이 기간에
원금을 갚는 구조가 아니라는 사실**을 그대로 보여주는 값이다.

그래서 하나로 뭉개지 않고 `llcr`(반환 최상위)과
`assumptions["llcr_noi_only"]` 를 함께 출고한다. **둘 중 하나만 인용하지 말 것.**
...
_DECISIONS = (
    ...
    "D8 LLCR 은 두 값을 함께 낸다 — llcr 은 준공 시점에서 잔여 NOI 와 매각대금을 "
    "월이율(금리/12)로 할인해 대출원금으로 나눈 값이고(이 대출의 상환 재원이 "
    "매각대금이라 분자의 대부분이 매각가다 — 커버리지보다 exit-LTV 의 역수처럼 "
    "움직인다), llcr_noi_only 는 잔존가치를 뺀 잔여 NOI 만으로 같은 계산을 한 "
    "값이다(대주 관행에 가깝지만 유한 horizon 에서는 1 을 크게 밑돈다). 둘 중 "
    "하나만 인용하지 말 것.",
...
def pf_model(inputs: dict) -> dict:
    ...
    | margin      | profit ÷ total_cost (매각가 대비가 아니다)  | 0.3070642466…    |
    | equity_irr  | 분기 13개 → `fin_core.irr_annual`           | 0.3281989683…    |
    | llcr        | (잔여 NOI + 매각가)의 준공시점 PV ÷ 대출    | 1.6174433058…    |
    | ↳ noi_only  | 잔여 NOI 만의 PV ÷ 대출(assumptions 에)     | 0.0434211697…    |

    월별 인출·이자 스케줄과 이 값들의 손계산 유도는 **모듈 docstring** 에 있다.
    가정 아홉(D1~D9)도 그쪽이 원본이고, 반환에는 `assumptions["decisions"]` 로
    같은 문장이 실린다.
    ...
    **`equity_irr` 은 `None` 일 수 있다** — 자기자본이 0 이면 유출이 없어
    ...
    지어내지 않고 `None` 을 준다. **`llcr` 과 `assumptions["llcr_noi_only"]` 도
    대출이 0 이면 `None` 이다**(나눌 분모가 없다). 부르는 쪽이 `None` 을
    처리해야 한다.

    **LLCR 은 두 값을 함께 읽어야 한다**(D8) — `llcr` 은 매각대금까지 상환
    재원으로 세고(분자의 대부분이 매각가다), `assumptions["llcr_noi_only"]` 는
    계획 문면대로 잔여 NOI 만 센다. 하나만 인용하면 지표의 의미가 바뀐다.
    ...
    # D8 — 브리프 문면 그대로 잔존가치를 뺀 값. 대주 관행에 가깝지만 유한
    # horizon 이라 1 을 크게 밑돈다. 위 llcr 과 **함께** 읽어야 하는 값이다.
    llcr_noi_only = pv_noi / loan_won if loan_won > 0 else None
    ...
    return {
        ...
        "assumptions": {
            ...
            "notes": [
                ...
                f"LLCR 은 두 값이다(D8) — llcr {llcr if llcr is None else f'{llcr:.4f}'}"
                " 은 (잔여 NOI + 매각대금) ÷ 대출원금이고, llcr_noi_only "
                f"{llcr_noi_only if llcr_noi_only is None else f'{llcr_noi_only:.4f}'}"
                " 은 매각대금을 뺀 값이다. 앞의 값은 분자의 대부분이 매각가라 "
                "커버리지보다 exit-LTV 의 역수처럼 움직이고, 뒤의 값은 대주 관행"
                "(잔존가치 제외)에 가깝지만 유한 horizon 이라 1 을 크게 밑돈다.",
            ],
            "caveats": [
